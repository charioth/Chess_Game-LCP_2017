	...
	public void render(Graphics graph) {
		...
		for (int j = 0; j < 2; j++) {
			for (int i = 0, row, column, value; i < 16; i++) {
				...
				if (j == 0)
					graph.drawImage(Assets.whitePiece[value], board[row][column].getRenderPos().x,
							board[row][column].getRenderPos().y, pieceSize, pieceSize, null);
				else
					graph.drawImage(Assets.blackPiece[value], board[row][column].getRenderPos().x,
							board[row][column].getRenderPos().y, pieceSize, pieceSize, null);
			}
		}
	}
