...
public class BoardMovements {
	...
	private static void castlingValidMove(Piece piece, PieceList[] pieceBox, Square[][] board, List<Coordinates> validMoves, List<Coordinates> specialMoves)
	{
		...
		if((piece.getType() == PieceInfo.KING) && !piece.isMoved()) // if selectedPiece is a king and was not moved
		{
			...
			for(int sideStep = 1 ; sideStep != 0 ; sideStep = ((sideStep > 0)? -1 : 0)) // Look both sides
			{
				...
				if(!pieceBox[piece.getColor().value].getPieces()[rookID].isMoved() && (rookID != -1)) // If rook was not moved then try see if Castling is valid with it
				{	
					for(int counter = 0,  column = piece.getActualPosition().getColumn() + sideStep ; counter < 2 ; counter++, column += sideStep) // Start walking validating the path
					{
	...
	private static void castlingMovement(Piece piece, PieceList[] pieceBox, Square[][] board, Coordinates position, List<Coordinates> specialMoves)
	{
	...
	private static void elPassantValidMove(Piece piece, PieceList[] pieceBox, Square[][] board, List<Coordinates> validMoves, List<Coordinates> specialMoves)
	{
		...
		if(piece.getType() == PieceInfo.PAWN && elpassant) // If is a pawn and el passant is possible
		{
			...
			{				
				...
				{	
					Coordinates position = new Coordinates(piece.getActualPosition().getRow(), piece.getActualPosition().getColumn() - i); // Left side of selected Pawn
					if(position.getColumn() >= 0) // Look if left side is in board
					{
						if(lastPawn.equals(position) && board[lastPawn.getRow() + side][lastPawn.getColumn()].getPieceID() == -1) //See if last pawn moved is by his side
						{
							validateMovement(board, piece, piece.getColor(), pieceBox, specialMoves, lastPawn.getRow() + side, lastPawn.getColumn()); // Check if the king will be put at check if use el passant
	...
	private static void elPassantAttack(Piece piece, PieceList[] pieceBox, Square[][] board, int adversaryColor, Coordinates point, List<Coordinates> specialMoves)
	{	
		if(elpassant && piece.getType() == PieceInfo.PAWN) // if el passant is true and piece is a pawn
		{
			if(specialMoves.contains(point)) //see if the chosen movement is the special movement
			{
				...
				pieceBox[adversaryColor].getPieces()[board[deadPiece.getRow()][deadPiece.getColumn()].getPieceID()].setType(PieceInfo.DEAD); // Mark piece as dead
	...
	public static void movePiece(Piece piece, Square board[][], PieceList[] pieceBox,
			ColorInfo turn, int row, int column) {
